...
- 消耗局内胶能抽取壳壳、亮钉、浮浮或芽芽。
- 点击或拖动卡牌到塔位，史莱姆会自动索敌攻击。
- 两只同类型、同星级史莱姆可融合，最高四星。
...
`assets/asset-spec.json` 精确声明 126 张正式游戏 PNG；`assets/rig-parts.json` 另引用 8 张角色主图集和 8 张表情图集。构建会检查路径、尺寸、透明通道和体积，缺失或不合格时不会覆盖上一次成功产物。
