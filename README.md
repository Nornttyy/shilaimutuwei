...
- 公开入口固定加载生成的 PNG 分层骨骼素材；旧的 `?rig=vector` 参数在线上会被忽略，仅本机 `localhost` 调试时可临时启用。启动加载采用 15 秒单图超时、受控并发、一次失败重试和 20 秒前台等待上限，避免全部 89 张图片同时抢占连接或永远卡在加载页；一个角色的所有分层仍然必须完整到齐才会原子切换，分层延迟但生成版立绘已就绪时会优先显示立绘，只有相关图片全部不可用时才使用应急矢量绘制。
...
- 战斗反馈使用逐帧 Canvas 程序动画：命中光刺、冲击环、击退拖尾、碰撞回弹、黏液爆散、治疗旋叶与波次清场均由多个独立部件运动组成，不依赖单张特效图片淡出。
