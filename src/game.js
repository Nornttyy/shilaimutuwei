...
export class SlimeGame {
  ...
  drawWelcome(ctx) {
    ...
    if (page === 0) {
      drawSlime(ctx, VIEW.width / 2 - 145, rect.y + 270, 112, 'shell', { time: this.time, phase: 0 });
      drawSlime(ctx, VIEW.width / 2 - 48, rect.y + 274, 92, 'needle', { time: this.time, phase: 1 });
      drawSlime(ctx, VIEW.width / 2 + 50, rect.y + 274, 92, 'bubble', { time: this.time, phase: 2 });
      drawSlime(ctx, VIEW.width / 2 + 145, rect.y + 272, 98, 'sprout', { time: this.time, phase: 3 });
      ctx.save();
      ctx.fillStyle = PALETTE.inkSoft;
      ctx.font = '700 17px "PingFang SC", sans-serif';
      ctx.textAlign = 'center';
